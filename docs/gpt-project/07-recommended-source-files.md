...
머더미스터리 작업이 잦다면 추가 추천

- `src/types/murderMystery.d.ts`
  - 시나리오 스키마, 스냅샷 구조, 전용 소켓 이벤트 계약 진실원
- `src/pages/api/socket/services/murderMysteryScenarioService.ts`
  - 시나리오 로딩/정규화/검증 규칙 진실원
- `src/pages/api/socket/services/murderMysteryStateMachine.ts`
  - 실제 진행 엔진과 상태 전이 규칙 진실원
- `src/components/murderMystery/MurderMysteryGameScreen.tsx`
  - 현재 UI가 스냅샷을 어떤 방식으로 소비하는지 확인할 때
