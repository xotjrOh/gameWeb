...
import { validateRoomName, validateGameType, validateMaxPlayers, validateAlreadyJoinOtherRoom,
    validateRoom, validateRoomFull, validateGameAlreadyStarted, validateDuplicateNickname, validateNickname, validateCannotLeave, validateOnlyHostRemoveRoom,
...
const commonHandler = (io, socket) => {
    ...
    socket.on('join-room', ({ roomId, userName, sessionId }, callback) => {
        try {
            const room = validateRoom(roomId);
            const defaultPlayerData = _.cloneDeep(DEFAULT_PLAYER_DATA[room.gameType]);
