...
export default function RoundResultModal({ socket, roomId }) {
  ...
  const { statusInfo, rounds } = useSelector((state) => state.horse.gameData);
  ...
  const getPlayerSuccess = (horse) => {
    ...
    return lastBet === horse && results.find(result => result.horse === horse && result.progress === 2);
  };
        ...
        <div className="space-y-4">
          ...
          {results.length === 0 && (
            <div className="text-center py-4 md:py-6">
              <h2 className="text-xl md:text-2xl font-bold text-red-500 mb-4">😢 아무도 베팅하지 않았습니다 😢</h2>
