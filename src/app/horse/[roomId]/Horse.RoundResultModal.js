...
export default function RoundResultModal({ socket, roomId }) {
  ...
  const { statusInfo, rounds, positions } = useSelector(
  ...
  const getPlayerSuccess = (horse) => {
    const lastBet =
      statusInfo?.voteHistory?.[statusInfo.voteHistory.length - 1];
    return (
      lastBet === horse &&
            ...
            .map(({ horse, progress }, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {horse} ({horsePosition - progress} → {horsePosition}) {isSuccess ? '🎉' : ''}
                  </span>
                  {/* 트랙 */}
