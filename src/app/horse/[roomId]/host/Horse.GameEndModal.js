...
export default function GameEndModal({ socket, roomId }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-dark bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96 text-center" ref={resultPopupRef}>
        ...
        <div className="mb-4">
          {gameResult.winners.length > 0 && (
            <>
              <h3 className="text-2xl font-bold text-green-500">🎉 우승한 말 🎉</h3>
        ...
        <div className="mb-4">
          {gameResult.losers.length > 0 && (
            <>
              <h3 className="text-2xl font-bold text-red-500">😢 패배한 말 😢</h3>
        ...
        {isMyHorseWinner ? (
          <div className="my-winner-message text-blue-500 text-xl font-bold">
            🎉 내 말 {statusInfo.horse}가 우승했어요! 🎉
          </div>
        ) : (
          <div className="my-loser-message text-gray-500 text-xl">
