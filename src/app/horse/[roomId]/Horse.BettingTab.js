...
function BettingTab({ roomId, socket, session }) {
  ...
  return (
    <div className="space-y-6">
      {/* 타이머 및 베팅 */}
      <div className="text-center bg-gray-100 p-3 md:p-4 rounded-lg shadow-md">
      <div className="flex justify-center items-baseline">
          <h2 className="text-xl md:text-2xl font-bold">베팅</h2>
          {/* **칩개수 추가** */}
          <p className="text-sm text-gray-500 ml-2">(남은 칩 개수 : {statusInfo?.chips || 0})</p>
        </div>
        <p className="text-red-500 text-sm md:text-base">칩은 리필되지 않으니 아껴서 베팅해주세요. <br/>베팅하기 버튼을 누른 이후에는 수정이 불가합니다.</p>

        <div className="grid grid-cols-2 gap-4 md:gap-6 mt-4">
          {horses.map((horse) => (
            <div key={horse} className="flex flex-col items-center bg-white p-3 md:p-4 rounded-lg shadow-sm border border-gray-300">
      ...
      <div className="bg-gray-100 p-3 md:p-4 rounded-lg shadow-md">
        <h3 className="text-lg md:text-xl font-bold mb-4">내 베팅 내역</h3>
            ...
            <div key={roundIndex} className="mb-4 md:mb-6">
              ...
              <div className="space-y-2">
                {round.map((bet, betIndex) => (
                  <div key={betIndex} className="flex justify-between items-center p-2 md:p-3 bg-white rounded-lg shadow-sm border border-gray-300">
